The driving simulator experiment tried to improve this paper \cite{driving}. It
was conducted mainly by Dr Nidhi Goyal, Dr Sunder Bukya and Dr Shiva Ram Reddy.
Its goal was to capture the attention drift of subject driving and being
distracted by events that can occur in real life.

For this experiment Gaspard Lefort and I helped set up the
...
centered or not). I gave ideas on how to process the gaze with a computer vision
algorithm because it was only recorded as a hotspot on the video capture of the
screen during the run. \cref{fig:driving-paper} (FROM \cite{driving}) shows the gaze density heatmap
from the original study \cite{driving}.

\insertfigure
    {assets/images/driving-paper}
    {height=0.3\textheight}
    {(IMAGE FROM \cite{driving}) Gaze density heatmap from the origial study in different scenarios}
    {fig:diffusion-mnist}
